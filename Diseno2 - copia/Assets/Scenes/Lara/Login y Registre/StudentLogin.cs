...
public class StudentLogin : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        userManager = UserManager.Instance; // Inicializar UserManager
        if (userManager == null)
        {
    ...
    public void LoginStudent()
    {
        ...
        auth.SignInWithEmailAndPasswordAsync(email, password).ContinueWith(task => {
            ...
            FetchAndLogStudentData(user.UserId);
        ...
        try
        {
            ...
            if (snapshot.Exists)
            {
                ...
                if (userData.TryGetValue("Rol", out object rol) && rol.ToString() == "Estudiante")
                {
                    ...
                    userManager.SetStudentData(userData);
                    Debug.Log("Datos del estudiante guardados en UserManager");
    ...
    {
        ShowSuccess("Inicio de sesión exitoso. ¡Bienvenido!");
        // Asegurarse de que el usuario actual esté configurado en UserManager
        userManager.SetCurrentUser(auth.CurrentUser);
    ...
    private async Task FetchAndLogStudentData(string userId)
    {
        try
        {
            Debug.Log($"Fetching teacher data for user: {userId}");

            DocumentReference teacherRef = db.Collection("Estudiantes").Document(userId);
